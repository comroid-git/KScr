...
public abstract class AbstractBytecodeAdapter : IBytecodePort
{
    ...
    public void Write(Stream stream, StringCache strings, IBytecode bytecode)
    {
        ...
        switch (bytecode)
        {
            ...
            case LiteralBytecode<string> str:
                ...
                break;
            default:
                throw new ArgumentOutOfRangeException(nameof(bytecode.ElementType), bytecode.ElementType,
                    "Unknown bytecode type");
        }
    ...
    protected void Write(Stream stream, BytecodeElementType type)
    {
        stream.Write(new[] { (byte)type });
    }

    protected void Write(Stream stream, byte b)
    {
        Write(stream, new[] { b }, BytecodeElementType.Byte);
    }

    protected void Write(Stream stream, int i)
    {
        Write(stream, BitConverter.GetBytes(i), BytecodeElementType.Int32);
    }
    ...
    {
        var type = ReadElementType(stream);
        ...
        switch (type)
        {
            ...
            case BytecodeElementType.Method:
                ...
                return (T)(object)ReadMethod(vm, stream, strings, pkg,
            ...
            case BytecodeElementType.MethodParameter:
                ...
                return (T)(object)ReadMethodParameter(vm, stream, strings, pkg,
                    cls ?? throw new NullReferenceException("Class cannot be null"));
            case BytecodeElementType.Property:
                ValidateBRT<Property, T>();
                return (T)(object)ReadProperty(vm, stream, strings, pkg,
                    cls ?? throw new NullReferenceException("Class cannot be null"));
            case BytecodeElementType.SourcePosition:
                ValidateBRT<SourcefilePosition, T>();
                return (T)(object)ReadSrcPos(vm, stream, strings, pkg,
                    cls ?? throw new NullReferenceException("Class cannot be null"));
            case BytecodeElementType.CodeBlock:
                ValidateBRT<ExecutableCode, T>();
                return (T)(object)ReadCode(vm, stream, strings, pkg,
                    cls ?? throw new NullReferenceException("Class cannot be null"));
            case BytecodeElementType.Statement:
                ValidateBRT<Statement, T>();
                return (T)(object)ReadStatement(vm, stream, strings, pkg,
                    cls ?? throw new NullReferenceException("Class cannot be null"));
    ...
    protected abstract Property ReadProperty(RuntimeBase vm, Stream stream, StringCache strings, Package pkg,
        Class cls);

    protected abstract Method ReadMethod(RuntimeBase vm, Stream stream, StringCache strings, Package pkg, Class cls);

    protected abstract MethodParameter ReadMethodParameter(RuntimeBase vm, Stream stream, StringCache strings,
        Package pkg, Class cls);

    protected abstract SourcefilePosition ReadSrcPos(RuntimeBase vm, Stream stream, StringCache strings, Package pkg,
        Class cls);

    protected abstract ExecutableCode ReadCode(RuntimeBase vm, Stream stream, StringCache strings, Package pkg,
        Class cls);

    protected abstract Statement ReadStatement(RuntimeBase vm, Stream stream, StringCache strings, Package pkg,
        Class cls);
    ...
    {
        return Read(stream, sizeof(byte), BytecodeElementType.Byte)[0];
    }

    protected int ReadInt(Stream stream)
    {
        return BitConverter.ToInt32(Read(stream, sizeof(int), BytecodeElementType.Int32));
    }

    protected uint ReadUInt(Stream stream)
    {
        return BitConverter.ToUInt32(Read(stream, sizeof(uint), BytecodeElementType.UInt32));
    }

    protected ulong ReadULong(Stream stream)
    {
        return BitConverter.ToUInt64(Read(stream, sizeof(ulong), BytecodeElementType.UInt64));
    }

    protected string ReadString(Stream stream, StringCache strings)
    {
        return strings[BitConverter.ToInt32(Read(stream, sizeof(int), BytecodeElementType.String))] ??
