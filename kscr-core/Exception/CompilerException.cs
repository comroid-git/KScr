...
public sealed class CompilerError
{
    ...
    public static readonly CompilerError ClassAbstractMemberNotImplemented = new("Class {0} does not implement the following abstract members:\n{1}");
    ...
    public string Format(object[] messageArgs) => string.Format(Message, messageArgs);
}

public class CompilerException : System.Exception, IStackTrace
{
    public CompilerException(SourcefilePosition srcPos, CompilerError error, params object?[] messageArgs /* expected, actual */) : base(error.Format(messageArgs))
    {
