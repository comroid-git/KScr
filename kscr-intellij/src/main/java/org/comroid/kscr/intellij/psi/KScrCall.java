...
import org.comroid.kscr.intellij.psi.types.JvmKScrMethod;
...
public class KScrCall extends KScrElement implements PsiReference{
	...
	public JvmMethod resolveMethod(){
		...
		KScrExpression on = PsiUtils.childOfType(getParent(), KScrExpression.class).orElse(null);
		...
		if(on != null)
			...
			var inMethod = PsiTreeUtil.getParentOfType(this, KScrMethod.class);
			var inType = PsiTreeUtil.getParentOfType(this, KScrType.class);
			if(inType != null)
				candidates = (inMethod != null && inMethod.isStatic())
						? inType.methods().stream().filter(KScrMethod::isStatic).map(JvmKScrMethod::of).collect(Collectors.toList())
						: inType.methods().stream().map(JvmKScrMethod::of).collect(Collectors.toList());
			else
